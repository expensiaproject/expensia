...
export default function EditReport() {
  ...
  const { data: allExpenses = [], isLoading: expensesLoading } = useQuery({
    queryKey: ['myAllExpenses', user?.id],
  ...
  // Load existing linked expenses
  useEffect(() => {
    if (allExpenses.length > 0 && reportId) {
      const linkedIds = allExpenses.filter(e => e.reportId === reportId).map(e => e.id);
      setSelectedExpenseIds(linkedIds);
    }
  }, [allExpenses, reportId]);

  // Available expenses: submitted without report OR already linked to this report
  const availableExpenses = allExpenses.filter(e => 
    e.status === 'submitted' && (!e.reportId || e.reportId === reportId)
  );
  ...
  const updateReportMutation = useMutation({
    mutationFn: async (reportData) => {
      // Update report
      await base44.entities.Report.update(reportId, reportData);
      
      // Update expense links
      // First, unlink expenses that were removed
      const currentlyLinked = allExpenses.filter(e => e.reportId === reportId);
      for (const exp of currentlyLinked) {
        if (!selectedExpenseIds.includes(exp.id)) {
          await base44.entities.Expense.update(exp.id, { reportId: null });
        }
      }
      
      // Then, link newly selected expenses
      for (const expenseId of selectedExpenseIds) {
        const expense = allExpenses.find(e => e.id === expenseId);
        if (expense && expense.reportId !== reportId) {
          await base44.entities.Expense.update(expenseId, { reportId });
        }
      }
      
      await logAuditEvent(user, 'report', reportId, 'update', reportData);
    },
    onSuccess: () => {
  ...
  if (reportLoading || expensesLoading) {
    return (
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {!report ? 'Report not found.' : 'Only open reports can be edited.'}
          </AlertDescription>
        </Alert>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500">Update your expense report</p>
        </div>
      </div>
          ...
          <CardContent>
            {availableExpenses.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No available expenses</p>
              </div>
            ) : (
